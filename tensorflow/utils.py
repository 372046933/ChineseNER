...
def calculate(x, y, id2word, id2tag, res=[]):
    ...
    for i in range(len(x)):  #for every sen
        for j in range(len(x[0])):  #for every word
            ...
            elif id2tag[y[i][j]][0] == 'M' and len(entity) != 0 and entity[
...
def train(model, sess, saver, epochs, batch_size, data_train, data_test,
          id2word, id2tag):
    ...
    for epoch in range(epochs):
        for batch in range(batch_num):
            ...
            pre, _ = sess.run([model.viterbi_sequence, model.train_op],
                              feed_dict)
            acc = 0
            if batch % 200 == 0:
        ...
        if epoch % 3 == 0:
            ...
            for batch in range(batch_num):
                ...
                entityall = calculate(x_batch, y_batch, id2word, id2tag,
                                      entityall)
...
def extraction(input_path, output_path, model, sess, word2id, id2tag,
               batch_size):
    text_id = []
    text = []
